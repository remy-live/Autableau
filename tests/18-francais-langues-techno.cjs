...
module.exports = async function (browser) {
    ...
    // ================================================================
    //
    // La plomberie, éprouvée sans rien télécharger : une FAUSSE voix rangée
    // dans le stockage, et un FAUX synthétiseur qui note ce qu'on lui donne à
    // dire. Le vrai moteur n'est pas là — c'est justement ce qu'il faut
    // éprouver : que son absence ne rende pas la dictée muette.
    // La marche à suivre pour le brancher est dans « docs/voix-piper.md ».
    const fausseVoix = `(() => ({
        nom: 'fr_FR-siwis-medium', langue: 'fr-FR',
        config: { sample_rate: 22050, dataset: 'siwis' },
        modele: new ArrayBuffer(200000)
    }))()`;

    const installee = await page.evaluate(async (src) => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        const attendre = (ms) => new Promise(ok => setTimeout(ok, ms));
        D.ouvrir();
        await attendre(200);
        const liste = D.widgetEl.querySelector('#dic-voix');
        const etat = D.widgetEl.querySelector('#dic-voix-locale-etat');
        // L'ordinateur d'essai n'a pas forcément de voix : on en pose une,
        // pour que « la voix installée passe devant » ait un devant. Et on
        // prend la MEILLEURE des voix du système — une voix améliorée — sans
        // quoi le classement ne prouverait rien : à rang égal, la voix
        // installée resterait en tête par le seul ordre du tableau.
        const vrai = D.moteur;
        D.moteur = Object.assign({}, vrai, {
            disponible: () => true,
            voix: () => [{ name: 'Amélie (Premium)', lang: 'fr-FR' }]
        });

        await D.retirerLaVoixLocale();
        D.majLaVoixLocale('');
        const avant = { options: [...liste.options].map(o => o.value),
                        etat: etat.textContent,
                        retirerEteint: D.widgetEl.querySelector('#dic-voix-retirer').disabled };

        await D.rangerLaVoixLocale(eval(src));
        D.majLaVoixLocale('');
        const apres = { options: [...liste.options].map(o => o.value),
                        etiquette: liste.options[0].textContent,
                        choisie: D.voixChoisie().name,
                        locale: !!D.voixChoisie().locale,
                        etat: etat.textContent,
                        retirerEteint: D.widgetEl.querySelector('#dic-voix-retirer').disabled };

        // ELLE SURVIT À LA FERMETURE. C'est tout l'intérêt de l'installer :
        // on la désigne une fois, elle est là l'année suivante.
        D.voixLocale = null;
        await D.lireLaVoixLocale();
        const relue = { nom: D.voixLocale && D.voixLocale.nom,
                        octets: D.voixLocale && D.voixLocale.octets };

        // UNE FICHE ABÎMÉE VAUT UNE ABSENCE : elle paraîtrait dans la liste, et
        // l'on choisirait une voix qui ne peut pas parler.
        const refus = [];
        for (const [quoi, fiche] of [
            ['sans modèle', { nom: 'X', config: { sample_rate: 22050 } }],
            ['modèle tronqué', { nom: 'X', config: { sample_rate: 22050 }, modele: new ArrayBuffer(12) }],
            ['sans fiche technique', { nom: 'X', modele: new ArrayBuffer(200000) }],
            ['fiche sans fréquence', { nom: 'X', config: {}, modele: new ArrayBuffer(200000) }],
            ['sans nom', { nom: '  ', config: { sample_rate: 22050 }, modele: new ArrayBuffer(200000) }]
        ]) {
            let pris = true;
            try { await D.rangerLaVoixLocale(fiche); } catch (e) { pris = false; }
            refus.push(quoi + ' : ' + (pris ? 'acceptée' : 'refusée'));
        }
        // Le refus n'a pas effacé la bonne.
        await D.lireLaVoixLocale();
        const encoreLa = D.voixLocale && D.voixLocale.nom;

        D.moteur = vrai;
        return { avant, apres, relue, refus, encoreLa };
    }, fausseVoix);

    r.verifie('sans voix installée, la zone le dit et « Retirer » est éteint',
        installee.avant.retirerEteint && /aucune/i.test(installee.avant.etat),
        JSON.stringify(installee.avant));
    r.egal('une voix installée passe même devant une voix améliorée',
        installee.apres.options, ['fr_FR-siwis-medium', 'Amélie (Premium)']);
    r.verifie('et la liste dit d\'où elle vient',
        /installée, hors ligne/.test(installee.apres.etiquette), installee.apres.etiquette);
    r.verifie('c\'est elle qu\'on prend d\'office',
        installee.apres.choisie === 'fr_FR-siwis-medium' && installee.apres.locale,
        JSON.stringify(installee.apres));
    r.verifie('la zone annonce son poids et où elle est rangée',
        /0,2 Mo/.test(installee.apres.etat) && /navigateur/.test(installee.apres.etat),
        installee.apres.etat);
    r.egal('elle survit à la fermeture de la fenêtre',
        installee.relue, { nom: 'fr_FR-siwis-medium', octets: 200000 });
    r.egal('une fiche abîmée est refusée, quelle que soit la façon', installee.refus, [
        'sans modèle : refusée', 'modèle tronqué : refusée', 'sans fiche technique : refusée',
        'fiche sans fréquence : refusée', 'sans nom : refusée']);
    r.egal('et un refus n\'emporte pas la voix déjà rangée',
        installee.encoreLa, 'fr_FR-siwis-medium');

    // ---- QUI PARLE VRAIMENT ----
    //
    // C'est la vérification qui compte : la dictée doit passer par le moteur
    // de la voix choisie, et non par « speechSynthesis » parce que c'est le
    // seul qu'elle connaisse.
    const quiParle = await page.evaluate(async () => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        const attendre = (ms) => new Promise(ok => setTimeout(ok, ms));
        const ditsNavigateur = [];
        const vrai = D.moteur;
        D.moteur = Object.assign({}, vrai, {
            disponible: () => true,
            voix: () => [{ name: 'Julie', lang: 'fr-FR' }],
            dire: (texte, opts, fini) => { ditsNavigateur.push(texte); setTimeout(fini, 0); return {}; },
            taire: () => {}
        });

        // 1. LE SYNTHÉTISEUR EST LÀ : c'est lui qui parle.
        const ditsInstallee = [];
        window.VoixInstallee = {
            dire: (texte, opts, fini) => { ditsInstallee.push(texte); setTimeout(fini, 0); return {}; },
            taire: () => {}
        };
        D.majLesVoix();
        D.reglages.voix = 'fr_FR-siwis-medium';
        D.poserLeTexte('Les cigognes reviennent.');
        D.lancer('dictee');
        await attendre(400);
        D.arreter();
        const avecMoteur = { installee: ditsInstallee.length, navigateur: ditsNavigateur.length };

        // 2. LE SYNTHÉTISEUR N'EST PAS LÀ. La dictée ne doit pas défiler en
        //    silence : elle le dit, change de voix, et REDIT ce qui n'a pas
        //    été entendu.
        delete window.VoixInstallee;
        ditsNavigateur.length = 0;
        D.reglages.voix = 'fr_FR-siwis-medium';
        D.poserLeTexte('Les cigognes reviennent.');
        D.lancer('dictee');
        await attendre(400);
        const alerte = D.widgetEl.querySelector('#dic-sansvoix');
        const sansMoteur = {
            navigateur: ditsNavigateur.length,
            alerte: alerte.textContent,
            alerteVue: getComputedStyle(alerte).display !== 'none',
            voixRetenue: D.reglages.voix,
            phase: D.phase
        };
        D.arreter();

        // 3. « ARRÊTER » FAIT TAIRE LES DEUX. On vient peut-être de changer de
        //    moteur en cours de lecture.
        let tus = { navigateur: false, installee: false };
        D.moteur = Object.assign({}, D.moteur, { taire: () => { tus.navigateur = true; } });
        window.VoixInstallee = { dire: (t, o, f) => { setTimeout(f, 0); return {}; },
                                 taire: () => { tus.installee = true; } };
        D.taireToutesLesVoix();
        delete window.VoixInstallee;

        D.moteur = vrai;
        await D.retirerLaVoixLocale();
        D.fermer();
        return { avecMoteur, sansMoteur, tus };
    });

    r.verifie('la voix installée choisie, c\'est son synthétiseur qui parle',
        quiParle.avecMoteur.installee > 0 && quiParle.avecMoteur.navigateur === 0,
        JSON.stringify(quiParle.avecMoteur));
    r.verifie('sans synthétiseur, la dictée ne part pas en silence : elle le dit',
        quiParle.sansMoteur.alerteVue && /synthétiseur/.test(quiParle.sansMoteur.alerte),
        JSON.stringify(quiParle.sansMoteur));
    r.verifie('et elle continue, avec une voix de l\'ordinateur',
        quiParle.sansMoteur.navigateur > 0 && quiParle.sansMoteur.voixRetenue === 'Julie',
        JSON.stringify(quiParle.sansMoteur));
    r.verifie('sans s\'arrêter au milieu de la dictée',
        quiParle.sansMoteur.phase !== 'arret', quiParle.sansMoteur.phase);
    r.egal('« Arrêter » fait taire les deux moteurs',
        quiParle.tus, { navigateur: true, installee: true });

    // ---- LE BOUTON, CLIQUÉ POUR DE BON ----
    //
    // Un « input » de fichier ne se déguise pas en bouton : il se commande.
    // On éprouve donc le chemin entier — clic de souris sur « Installer une
    // voix… », sélecteur de fichiers, deux fichiers remis, voix rangée.
    await page.evaluate(async () => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        D.ouvrir();
        await new Promise(ok => setTimeout(ok, 200));
        D.plierLesReglages(true);
    });
    await page.waitForTimeout(150);
    const boite = await page.evaluate(() => {
        const b = document.getElementById('dic-voix-poser');
        b.scrollIntoView({ block: 'center' });
        const c = b.getBoundingClientRect();
        const centre = { x: Math.round(c.x + c.width / 2), y: Math.round(c.y + c.height / 2) };
        // Le point que l'on va cliquer répond-il bien à ce bouton ?
        const dessus = document.elementFromPoint(centre.x, centre.y);
        return Object.assign(centre, { decouvert: dessus === b || b.contains(dessus) });
    });
    r.verifie('le bouton « Installer une voix… » est cliquable là où on le voit',
        boite.decouvert, JSON.stringify(boite));
    // UNE ATTENTE QUI EXPIRE NE DOIT PAS EMPORTER LE CHAPITRE. Si le bouton ne
    // mène plus au sélecteur, on veut le lire en une ligne — et garder les
    // deux cents vérifications qui précèdent, dont aucune n'y est pour rien.
    let selecteur = null, ouverture = 'ouvert';
    try {
        [selecteur] = await Promise.all([
            page.waitForEvent('filechooser', { timeout: 5000 }),
            page.mouse.click(boite.x, boite.y)
        ]);
    } catch (e) { ouverture = 'aucun sélecteur ne s’est ouvert'; }
    r.egal('un clic sur « Installer une voix… » ouvre le sélecteur de fichiers',
        ouverture, 'ouvert');
    if (selecteur) {
        await selecteur.setFiles([
            { name: 'fr_FR-siwis-medium.onnx', mimeType: 'application/octet-stream',
              buffer: Buffer.alloc(200000, 7) },
            { name: 'fr_FR-siwis-medium.onnx.json', mimeType: 'application/json',
              buffer: Buffer.from(JSON.stringify({ sample_rate: 22050, dataset: 'siwis',
                                                   language: { code: 'fr_FR' } })) }
        ]);
    ...
    const posee = await page.evaluate(() => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        return { nom: D.voixLocale && D.voixLocale.nom,
                 langue: D.voixLocale && D.voixLocale.langue,
                 octets: D.voixLocale && D.voixLocale.octets,
                 dansLaListe: [...D.widgetEl.querySelectorAll('#dic-voix option')].map(o => o.value),
                 etat: D.widgetEl.querySelector('#dic-voix-locale-etat').textContent };
    });
    r.egal('le bouton mène au sélecteur, et les deux fichiers font une voix',
        { nom: posee.nom, langue: posee.langue, octets: posee.octets },
        { nom: 'siwis', langue: 'fr-FR', octets: 200000 });
    r.verifie('qui paraît aussitôt dans la liste des voix',
        posee.dansLaListe[0] === 'siwis', JSON.stringify(posee.dansLaListe));
    // UN SEUL FICHIER NE FAIT PAS UNE VOIX, et on le dit plutôt que d'échouer
    // sans rien : c'est un geste qu'on ne fait qu'une fois par an.
    const seul = await page.evaluate(async () => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        await D.installerDepuisLesFichiers([new File([new ArrayBuffer(200000)],
            'fr_FR-siwis-medium.onnx', { type: 'application/octet-stream' })]);
        return D.widgetEl.querySelector('#dic-voix-locale-etat').textContent;
    });
    r.verifie('un seul des deux fichiers, et l\'on dit lequel manque',
        /\.onnx\.json/.test(seul) && /deux/.test(seul), seul);
    // ET « RETIRER », CLIQUÉ AUSSI : une voix qu'on ne peut pas enlever est
    // une voix qu'on n'installe pas.
    // ON LA CHOISIT D'ABORD DANS LA LISTE, comme on le ferait : sans cela, le
    // réglage désignerait encore une autre voix et « le retrait nettoie le
    // réglage » serait vrai sans rien faire.
    await page.selectOption('#dic-voix', 'siwis');
    await page.waitForTimeout(150);
    const choisieAvant = await page.evaluate(() =>
        PluginManager.plugins['lecteurDicteeTool'].reglages.voix);
    r.egal('choisir la voix installée dans la liste la retient', choisieAvant, 'siwis');
    const boiteRetirer = await page.evaluate(() => {
        const b = document.getElementById('dic-voix-retirer');
        const c = b.getBoundingClientRect();
        return { x: Math.round(c.x + c.width / 2), y: Math.round(c.y + c.height / 2),
                 eteint: b.disabled };
    });
    r.egal('« Retirer » s\'allume quand il y a quelque chose à retirer', boiteRetirer.eteint, false);
    await page.mouse.click(boiteRetirer.x, boiteRetirer.y);
    await page.waitForTimeout(400);
    const retiree = await page.evaluate(() => {
        const D = PluginManager.plugins['lecteurDicteeTool'];
        const etat = { restante: D.voixLocale,
                       dansLaListe: [...D.widgetEl.querySelectorAll('#dic-voix option')].map(o => o.value),
                       message: D.widgetEl.querySelector('#dic-voix-locale-etat').textContent,
                       reglage: D.reglages.voix };
        D.fermer();
        return etat;
    });
    r.egal('un clic sur « Retirer » l\'enlève pour de bon', retiree.restante, null);
    r.verifie('elle quitte la liste des voix',
        !retiree.dansLaListe.includes('siwis'), JSON.stringify(retiree.dansLaListe));
    r.verifie('et le réglage ne désigne plus une voix absente',
        retiree.reglage !== 'siwis', retiree.reglage);
