...
module.exports = async function (browser) {
    ...
    const zoomEnPresentation = await page.evaluate(async () => {
        ...
        // LE ZOOM S'ANIME SUR « requestAnimationFrame », ET ON NE L'ATTEND PLUS.
        //
        // On a d'abord compté les millisecondes, puis attendu que le zoom bouge
        // et se pose. Les deux reposaient sur la même chose : que le navigateur
        // serve des images d'animation. Un serveur chargé peut n'en servir
        // aucune pendant plusieurs secondes — le contrôle est alors tombé une
        // troisième fois, sur « plusGrand: false », pour une raison qui n'a
        // rien à voir avec ce qu'il éprouve.
        //
        // On DÉROULE donc l'animation à la main. C'est le même code que celui
        // qu'appelle le navigateur, la même arrivée — « zoomVise » retombe à
        // rien quand le zoom s'est posé —, sans dépendre de son horloge. Et si
        // la molette n'a rien visé du tout, la boucle ne tourne pas : c'est
        // alors la panne que ce contrôle doit dire, et il la dira.
        for (let i = 0; i < 300 && zoomVise !== null; i++) glisserLeZoom();
