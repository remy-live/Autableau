...
(function () {
    ...
    const drive = {
        ...
        // « grand » : la carte de survol montre l'image en grand, là où la
        // liste se contente d'une vignette. Drive porte la taille dans
        // l'adresse même de la vignette — il suffit de la redemander.
        async apercu(fichier, grand) {
            if (!fichier.vignette) return null;
            try {
                const adresse = grand ? String(fichier.vignette).replace(/=s\d+$/, '=s800') : fichier.vignette;
                const r = await this.appeler(adresse);
                if (!r.ok) return null;
