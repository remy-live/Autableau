...
(function () {
    ...
    // Types que le tableau sait poser. Les documents Google (Docs, Sheets…) ne
    // sont pas des fichiers : ils demandent une conversion, on les écarte.
    const TYPES_ACCEPTES = /^(image\/|application\/pdf|audio\/|video\/)/;
    ...
    class FournisseurDrive {
        ...
        async telecharger(fichier) {
            await this.connecter();
            const url = `https://www.googleapis.com/drive/v3/files/${fichier.id}?alt=media`;
            const r = await fetch(url, { headers: { Authorization: 'Bearer ' + this.jeton } });
