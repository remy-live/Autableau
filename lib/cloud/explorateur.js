...
(function () {
    ...
    function rendreBarre() {
        ...
        barre.innerHTML = `
            ...
            ${dansUnDossier ? `
                ...
                <button id="exp-vue" class="exp-bouton" title="Liste ou aperçus">${etat.vue === 'grille' ? '☰' : '▦'}</button>` : ''}`;
        ...
        if (vue) vue.addEventListener('click', () => {
            etat.vue = etat.vue === 'grille' ? 'liste' : 'grille';
    ...
    function rendreAccueilOrdinateur() {
        ...
        el('exp-corps').innerHTML = `
            <div id="exp-depot">
                ...
                ${arbre ? '<div class="exp-depot-note">Ouvrir un dossier permet de le parcourir ici, sans repasser par la fenêtre du système.</div>'
                        : '<div class="exp-depot-note">Ce navigateur ne sait pas parcourir un dossier dans la page : la fenêtre du système s\\'ouvrira.</div>'}
            </div>`;
    ...
    function rendreCorps() {
        ...
        corps.innerHTML = '';
        corps.className = etat.vue === 'grille' ? 'exp-grille' : '';
        liste.forEach(f => corps.appendChild(etat.vue === 'grille' ? carte(f) : ligne(f)));
        observerApercus();
    }
