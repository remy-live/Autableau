// ============================================================
// L'EXPLORATEUR DE FICHIERS
// ============================================================
// Une fenêtre déplaçable, redimensionnable et réductible, avec à gauche les
// SOURCES et à droite ce qu'elles contiennent. L'ordinateur est toujours là ;
// les autres (Google Drive, et demain Dropbox ou Nextcloud) s'ajoutent en
// écrivant un « fournisseur » et en l'enregistrant :
//
//     Explorateur.enregistrer({
//         cle: 'dropbox', nom: 'Dropbox', icone: '📦',
//         dispo()        → true si la source peut servir ici,
//         raison()       → pourquoi elle ne peut pas (affiché tel quel),
//         racine()       → { id, nom } du dossier de départ,
//         lister(id)     → [{ id, nom, dossier, type, taille, date }]  (Promise)
//         telecharger(f) → un File  (Promise)
//         apercu(f)      → une URL d'image, ou null  (Promise, facultatif)
//         configurer(zone, fait) → dessine un formulaire de réglage dans
//                          « zone » quand dispo() est faux, et appelle
//                          « fait » une fois la source prête (facultatif)
//     });
//
// Rien d'autre n'est demandé : la fenêtre, la navigation, la recherche, les
// aperçus et la remise du fichier à l'application sont écrits une seule fois,
// ici.
// ============================================================
(function () {
    'use strict';

    const CLE_ETAT = 'board_explorateur';
    const MIN_L = 460, MIN_H = 320;

    const etat = {
        x: 140, y: 80, w: 720, h: 500,
        vue: 'liste',        // 'liste', 'details', 'apercus' ou 'mosaique'
        tri: 'nom',          // 'nom', 'date', 'taille'
        source: 'ordi',
        reduite: false
    };
    try { Object.assign(etat, JSON.parse(localStorage.getItem(CLE_ETAT) || '{}')); } catch (e) { /* stockage refusé */ }
    function retenir() {
        try { localStorage.setItem(CLE_ETAT, JSON.stringify(etat)); } catch (e) { /* stockage refusé */ }
    }

    const fournisseurs = [];
    let chemin = [];             // fil d'Ariane de la source courante
    let fichiers = [];           // ce que la source a renvoyé
    let recherche = '';
    let apercus = null;          // observateur de visibilité pour les vignettes

    const el = (id) => document.getElementById(id);
    const courant = () => fournisseurs.find(f => f.cle === etat.source) || fournisseurs[0];

    // ---------------------------------------------------------
    // LA SOURCE « MON ORDINATEUR »
    // Les navigateurs récents savent ouvrir un dossier et le parcourir
    // (File System Access). Là où ce n'est pas possible, on retombe sur le
    // dépôt de fichiers et la fenêtre du système.
    // ---------------------------------------------------------
    const ordinateur = {
        cle: 'ordi', nom: 'Mon ordinateur', icone: '💻',
        // Ses fichiers sont déjà là : on ne propose pas de les télécharger.
        local: true,
        dossierRacine: null,
        dispo: () => true,
        arborescencePossible: () => typeof window.showDirectoryPicker === 'function',
        racine() { return this.dossierRacine ? { id: this.dossierRacine, nom: this.dossierRacine.name } : null; },

        async choisirDossier() {
            // « startIn » ouvre le sélecteur dans les Documents plutôt qu'à la
            // racine du disque — justement là où le navigateur refuse d'aller.
            // « id » lui fait retenir le dernier dossier choisi pour ce usage.
            const poignee = await window.showDirectoryPicker({
                mode: 'read', id: 'autableau-cours', startIn: 'documents'
            });
            this.dossierRacine = poignee;
            return { id: poignee, nom: poignee.name };
        },

        async lister(poignee) {
            const res = [];
            for await (const [nom, entree] of poignee.entries()) {
                if (nom.startsWith('.')) continue;
                if (entree.kind === 'directory') { res.push({ id: entree, nom, dossier: true, type: '' }); continue; }
                let taille = 0, date = 0, type = '';
                try { const f = await entree.getFile(); taille = f.size; date = f.lastModified; type = f.type; } catch (e) { /* fichier illisible */ }
                res.push({ id: entree, nom, dossier: false, type, taille, date });
            }
            return res;
        },

        async telecharger(fichier) { return await fichier.id.getFile(); },

        async apercu(fichier) {
            if (!/^image\//.test(fichier.type || '')) return null;
            try { return URL.createObjectURL(await fichier.id.getFile()); } catch (e) { return null; }
        }
    };

    // ---------------------------------------------------------
    // CE QUE LE TABLEAU SAIT OUVRIR
    // ---------------------------------------------------------
    const TYPES_OK = /^(image\/|application\/pdf|audio\/|video\/|text\/)/;
    const EXT_OK = /\.(png|jpe?g|gif|webp|svg|bmp|avif|pdf|mp3|wav|ogg|m4a|aac|mp4|webm|mov|m4v|txt|md|csv|docx|odt)$/i;

    function importable(f) {
        if (f.dossier) return true;
        if (f.converti) return true;                                  // documents Google & co
        if (/^application\/vnd\.google-apps\./.test(f.type || '')) return false;
        return TYPES_OK.test(f.type || '') || EXT_OK.test(f.nom || '')
            || f.type === 'application/octet-stream' || !f.type;
    }

    function icone(f) {
        const nom = (f.nom || '').toLowerCase(), type = f.type || '';
        if (f.dossier) return '📁';
        if (/^image\//.test(type) || /\.(png|jpe?g|gif|webp|svg|bmp|avif)$/.test(nom)) return '🖼️';
        if (type === 'application/pdf' || /\.pdf$/.test(nom)) return '📕';
        if (/^audio\//.test(type) || /\.(mp3|wav|ogg|m4a|aac)$/.test(nom)) return '🎵';
        if (/^video\//.test(type) || /\.(mp4|webm|mov|m4v)$/.test(nom)) return '🎬';
        if (/wordprocessing|opendocument\.text|google-apps\.document/.test(type) || /\.(docx|odt)$/.test(nom)) return '📝';
        if (/spreadsheet|csv/.test(type) || /\.csv$/.test(nom)) return '📊';
        if (/presentation/.test(type)) return '📽️';
        if (/^text\//.test(type) || /\.(txt|md)$/.test(nom)) return '📃';
        return '📄';
    }

    function poidsLisible(octets) {
        if (!octets) return '';
        if (octets < 1024) return octets + ' o';
        if (octets < 1048576) return Math.round(octets / 1024) + ' Ko';
        return (octets / 1048576).toFixed(1).replace('.', ',') + ' Mo';
    }

    function dateLisible(ms) {
        if (!ms) return '';
        const d = new Date(ms);
        if (isNaN(d)) return '';
        return d.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit' });
    }

    function echapper(t) {
        return String(t == null ? '' : t).replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
    }

    // ---------------------------------------------------------
    // LA FENÊTRE
    // ---------------------------------------------------------
    function construire() {
        if (el('explorateur')) return;
        const f = document.createElement('div');
        f.id = 'explorateur';
        f.innerHTML = `
            <div id="exp-entete">
                <b id="exp-titre">🗂️ Ouvrir un fichier</b>
                <button id="exp-reduire" title="Réduire">—</button>
                <button id="exp-fermer" title="Fermer">✕</button>
            </div>
            <div id="exp-contenu">
                <div id="exp-sources"></div>
                <div id="exp-droite">
                    <div id="exp-barre"></div>
                    <div id="exp-raccourcis" style="display:none;"></div>
                    <div id="exp-corps"></div>
                </div>
            </div>
            <div id="exp-poignee" title="Redimensionner"></div>`;
        document.body.appendChild(f);
        appliquerGeometrie();
        // La fenêtre a sa poignée depuis toujours : elle ne prend que le plein écran.
        if (typeof window.equiperFenetre === 'function') {
            window.equiperFenetre(f, 'explorateur', { saPropreP: true });
        }

        deplacer(f, el('exp-entete'));
        redimensionner(f, el('exp-poignee'));

        el('exp-fermer').addEventListener('click', fermer);
        el('exp-reduire').addEventListener('click', reduire);

        // Le menu des trois points se referme comme tout menu : un appui à
        // côté, la touche d'échappement, ou la liste qui défile sous lui.
        document.addEventListener('click', fermerLeMenu);
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') { fermerLeMenu(); cacherLApercu(); }
        });
        el('exp-corps').addEventListener('scroll', () => { fermerLeMenu(); cacherLApercu(); });

        // Glisser un fichier DANS la fenêtre revient à l'ouvrir
        f.addEventListener('dragover', (e) => { e.preventDefault(); f.classList.add('exp-survol'); });
        f.addEventListener('dragleave', () => f.classList.remove('exp-survol'));
        f.addEventListener('drop', (e) => {
            e.preventDefault(); e.stopPropagation();
            f.classList.remove('exp-survol');
            const liste = Array.from((e.dataTransfer && e.dataTransfer.files) || []);
            if (liste.length) { remettre(liste); fermer(); }
        });
    }

    // Réduire, c'est ranger la fenêtre dans le dock des minimisations, en bas
    // à gauche, avec les palettes déjà rangées — plutôt que de la laisser en
    // bandeau au travers du tableau.
    function reduire() {
        const f = el('explorateur');
        if (!f) return;
        etat.reduite = true;
        retenir();
        f.style.display = 'none';
        if (typeof window.rangerDansLeDock === 'function') {
            window.rangerDansLeDock('explorateur', 'Rouvrir « Ouvrir un fichier »', '🗂️', () => ouvrir());
        } else {
            // Sans dock (vieille version), on retombe sur le bandeau
            f.style.display = 'flex';
            appliquerGeometrie();
        }
    }

    function appliquerGeometrie() {
        const f = el('explorateur');
        if (!f) return;
        etat.w = Math.max(MIN_L, Math.min(etat.w, window.innerWidth - 20));
        etat.h = Math.max(MIN_H, Math.min(etat.h, window.innerHeight - 20));
        etat.x = Math.max(0, Math.min(etat.x, window.innerWidth - 240));
        etat.y = Math.max(0, Math.min(etat.y, window.innerHeight - 60));
        f.style.left = etat.x + 'px';
        f.style.top = etat.y + 'px';
        f.style.width = etat.w + 'px';
        f.style.height = etat.h + 'px';
        f.classList.remove('exp-reduite');
        el('exp-reduire').innerText = '—';
        el('exp-reduire').title = 'Réduire dans la barre du bas';
    }

    function deplacer(fenetre, poignee) {
        let actif = false, dx = 0, dy = 0;
        poignee.addEventListener('pointerdown', (e) => {
            if (e.target.tagName === 'BUTTON') return;
            actif = true; dx = e.clientX - fenetre.offsetLeft; dy = e.clientY - fenetre.offsetTop;
            poignee.setPointerCapture(e.pointerId);
        });
        poignee.addEventListener('pointermove', (e) => {
            if (!actif) return;
            etat.x = e.clientX - dx; etat.y = e.clientY - dy;
            appliquerGeometrie();
        });
        const fin = () => { if (actif) { actif = false; retenir(); } };
        poignee.addEventListener('pointerup', fin);
        poignee.addEventListener('pointercancel', fin);
    }

    function redimensionner(fenetre, poignee) {
        let actif = false, x0 = 0, y0 = 0, l0 = 0, h0 = 0;
        poignee.addEventListener('pointerdown', (e) => {
            actif = true; x0 = e.clientX; y0 = e.clientY; l0 = fenetre.offsetWidth; h0 = fenetre.offsetHeight;
            poignee.setPointerCapture(e.pointerId);
            e.preventDefault();
        });
        poignee.addEventListener('pointermove', (e) => {
            if (!actif) return;
            etat.reduite = false;
            etat.w = l0 + (e.clientX - x0);
            etat.h = h0 + (e.clientY - y0);
            appliquerGeometrie();
        });
        const fin = () => { if (actif) { actif = false; retenir(); rendreCorps(); } };
        poignee.addEventListener('pointerup', fin);
        poignee.addEventListener('pointercancel', fin);
    }

    function fermer() {
        const f = el('explorateur');
        if (f) f.style.display = 'none';
        etat.reduite = false;
        retenir();
        cacherLApercu();
        fermerLeMenu();
        oublierLesApercus();
        // Fermer, ce n'est pas réduire : on ne laisse pas d'icône derrière soi.
        if (typeof window.retirerDuDock === 'function') window.retirerDuDock('explorateur');
        if (apercus) { apercus.disconnect(); apercus = null; }
    }

    // ---------------------------------------------------------
    // LES SOURCES
    // ---------------------------------------------------------
    function rendreSources() {
        const zone = el('exp-sources');
        zone.innerHTML = '';
        fournisseurs.forEach(f => {
            const dispo = f.dispo();
            const b = document.createElement('button');
            const aRelier = !dispo && typeof f.configurer === 'function';
            b.className = 'exp-source' + (etat.source === f.cle ? ' actif' : '')
                + (dispo || aRelier ? '' : ' indispo');
            if (aRelier) b.title = 'À relier — cliquez pour donner votre adresse';
            b.dataset.source = f.cle;
            b.innerHTML = `<span class="exp-source-icone">${f.icone}</span><span>${echapper(f.nom)}</span>`;
            b.addEventListener('click', () => allerA(f.cle));
            zone.appendChild(b);
        });
        const aide = document.createElement('div');
        aide.className = 'exp-aide';
        aide.innerHTML = 'Images, PDF, sons, vidéos,<br>Word, LibreOffice, texte.';
        zone.appendChild(aide);
    }

    async function allerA(cle) {
        etat.source = cle;
        retenir();
        chemin = [];
        recherche = '';
        rendreSources();
        await ouvrirRacine();
    }

    async function ouvrirRacine() {
        const source = courant();
        if (!source) return;
        if (!source.dispo()) {
            rendreBarre();
            // Une source peut n'être qu'« à configurer » : elle propose alors
            // son propre formulaire au lieu d'un simple message d'échec.
            if (typeof source.configurer === 'function') {
                el('exp-corps').innerHTML = '';
                source.configurer(el('exp-corps'), () => { rendreSources(); ouvrirRacine(); });
                return;
            }
            return message(source.raison ? source.raison() : 'Source indisponible.');
        }
        const racine = await source.racine();
        if (!racine) { rendreBarre(); return rendreAccueilOrdinateur(); }
        chemin = [racine];
        await charger();
    }

    // ---------------------------------------------------------
    // LA BARRE : retour, chemin, recherche, tri, vue
    // ---------------------------------------------------------
    function rendreBarre() {
        const barre = el('exp-barre');
        const dansUnDossier = chemin.length > 0;
        barre.innerHTML = `
            ${chemin.length > 1 ? '<button id="exp-retour" class="exp-bouton">← Retour</button>' : ''}
            <span id="exp-chemin">${chemin.map(c => echapper(c.nom)).join(' › ') || echapper(courant() ? courant().nom : '')}</span>
            ${dansUnDossier ? `
                <button id="exp-raccourci" class="exp-bouton${raccourciCourant() ? ' actif' : ''}"
                        title="${raccourciCourant() ? 'Retirer ce dossier des raccourcis' : 'Garder ce dossier en raccourci'}">${raccourciCourant() ? '★' : '☆'}</button>
                <input id="exp-recherche" type="search" placeholder="Rechercher…" value="${echapper(recherche)}">
                <select id="exp-tri" title="Trier">
                    <option value="nom">Nom</option>
                    <option value="date">Date</option>
                    <option value="taille">Taille</option>
                </select>
                <button id="exp-vue" class="exp-bouton" title="Présentation : ${VUES[etat.vue].nom} (cliquer pour changer)">${VUES[etat.vue].icone}</button>` : ''}`;

        if (el('exp-retour')) el('exp-retour').addEventListener('click', remonter);
        if (el('exp-raccourci')) el('exp-raccourci').addEventListener('click', basculerRaccourci);
        const rech = el('exp-recherche');
        if (rech) rech.addEventListener('input', () => { recherche = rech.value; rendreCorps(); });
        const tri = el('exp-tri');
        if (tri) {
            tri.value = etat.tri;
            tri.addEventListener('change', () => { etat.tri = tri.value; retenir(); rendreCorps(); });
        }
        const vue = el('exp-vue');
        if (vue) vue.addEventListener('click', () => {
            const ordre = Object.keys(VUES);
            etat.vue = ordre[(ordre.indexOf(etat.vue) + 1) % ordre.length];
            retenir(); rendreBarre(); rendreCorps();
            if (typeof showToast === 'function') showToast('Présentation : ' + VUES[etat.vue].nom);
        });
    }

    // Quatre façons de regarder un dossier : la liste sobre, le tableau
    // détaillé, les aperçus, et la mosaïque pour choisir une image à l'œil.
    const VUES = {
        liste:    { nom: 'Liste', icone: '☰', classe: '' },
        details:  { nom: 'Détails', icone: '▤', classe: 'exp-details' },
        apercus:  { nom: 'Aperçus', icone: '▦', classe: 'exp-grille' },
        mosaique: { nom: 'Mosaïque', icone: '⬛', classe: 'exp-grille exp-mosaique' }
    };

    function message(texte, couleur) {
        el('exp-corps').innerHTML = `<div class="exp-message" ${couleur ? `style="color:${couleur}"` : ''}>${texte}</div>`;
    }

    // ---------------------------------------------------------
    // L'ACCUEIL DE L'ORDINATEUR : dépôt, parcourir, ouvrir un dossier
    // ---------------------------------------------------------
    // Le bouton PRINCIPAL était « Parcourir mes fichiers… », c'est-à-dire
    // justement celui qui renvoie à la fenêtre du système et referme la nôtre.
    // Quand le navigateur sait parcourir un dossier dans la page, c'est ÇA
    // l'action principale, et on dit clairement ce qui va se passer : une seule
    // fenêtre du système pour autoriser le dossier, puis tout se fait ici.
    function rendreAccueilOrdinateur(remarque) {
        const arbre = ordinateur.arborescencePossible();
        el('exp-corps').innerHTML = `
            <div id="exp-depot">
                <div class="exp-depot-icone">📥</div>
                <div class="exp-depot-titre">Glissez un fichier ici</div>
                <div class="exp-depot-ou">ou</div>
                <div class="exp-depot-boutons">
                    ${arbre ? '<button id="exp-dossier" class="exp-primaire">📂 Parcourir un dossier ici</button>' : ''}
                    <button id="exp-parcourir" class="${arbre ? 'exp-bouton' : 'exp-primaire'}">Choisir un fichier…</button>
                </div>
                ${remarque ? `<div class="exp-depot-remarque">${remarque}</div>` : ''}
                <div class="exp-depot-note">${arbre
                    ? "« Parcourir un dossier ici » : le navigateur demande une fois l'autorisation d'ouvrir le dossier — c'est sa fenêtre à lui, on ne peut pas l'éviter. Ensuite, tout se passe dans cette fenêtre-ci : dossiers, aperçus, recherche."
                    : "Ce navigateur ne sait pas parcourir un dossier dans la page (essayez Chrome ou Edge) : la fenêtre du système s'ouvrira."}</div>
            </div>`;
        el('exp-parcourir').addEventListener('click', () => {
            const entree = document.getElementById('pdf-loader');
            if (entree) entree.click();
            fermer();
        });
        if (el('exp-dossier')) el('exp-dossier').addEventListener('click', async () => {
            try {
                message('Autorisation du dossier…');
                const racine = await ordinateur.choisirDossier();
                chemin = [racine];
                await charger();
            } catch (e) {
                // Le navigateur rend la même erreur qu'on ait annulé ou qu'il
                // ait refusé le dossier : impossible de distinguer les deux.
                // On rappelle donc, sans accuser, quels dossiers il refuse —
                // son message (« contains system files ») est le sien, et il
                // est trompeur : ce n'est pas le contenu du dossier qui est en
                // cause, mais sa place dans l'arborescence.
                rendreAccueilOrdinateur(
                    "Le navigateur refuse d'ouvrir certains dossiers, quel que soit le site : "
                    + "la racine d'un disque, votre dossier personnel, et les dossiers du système. "
                    + "Son message parle de « fichiers système », mais c'est bien l'emplacement qui "
                    + "le gêne, pas le contenu. Choisissez un dossier plus bas — "
                    + "<b>Documents ▸ Cours</b> par exemple — et tout ira bien.");
            }
        });
    }

    // ---------------------------------------------------------
    // LA LISTE
    // ---------------------------------------------------------
    async function charger() {
        rendreBarre();
        rendreRaccourcis();
        oublierLesApercus();
        message('Chargement…');
        try {
            fichiers = await courant().lister(chemin[chemin.length - 1].id);
            rendreCorps();
        } catch (e) {
            message('Erreur : ' + echapper(e.message || e), '#d63031');
        }
    }

    function trier(liste) {
        const cle = etat.tri;
        return liste.slice().sort((a, b) => {
            if (a.dossier !== b.dossier) return a.dossier ? -1 : 1;
            if (cle === 'date') return (b.date || 0) - (a.date || 0);
            if (cle === 'taille') return (b.taille || 0) - (a.taille || 0);
            return String(a.nom).localeCompare(String(b.nom), 'fr', { numeric: true });
        });
    }

    function rendreCorps() {
        if (!chemin.length) return;
        // La carte d'aperçu et le menu vivent sur la page, pas dans la liste :
        // refaire la liste sous eux les laisserait flotter au-dessus du vide.
        cacherLApercu();
        fermerLeMenu();
        const corps = el('exp-corps');
        const filtre = recherche.trim().toLowerCase();
        const liste = trier(fichiers.filter(f => !filtre || String(f.nom).toLowerCase().includes(filtre)));

        if (!liste.length) return message(filtre ? 'Aucun fichier ne porte ce nom.' : 'Ce dossier est vide.');

        corps.innerHTML = '';
        corps.className = (VUES[etat.vue] || VUES.liste).classe;
        if (etat.vue === 'details') corps.appendChild(enteteDetails());
        liste.forEach(f => corps.appendChild(
            etat.vue === 'apercus' || etat.vue === 'mosaique' ? carte(f)
                : etat.vue === 'details' ? ligneDetaillee(f) : ligne(f)));
        observerApercus();
    }

    function meta(f) {
        if (f.dossier) return 'Dossier';
        if (!importable(f)) return 'non pris en charge';
        return [poidsLisible(f.taille), dateLisible(f.date)].filter(Boolean).join(' · ') || 'Fichier';
    }

    function brancher(element, f) {
        if (!importable(f)) { element.classList.add('exp-inactif'); return element; }
        element.addEventListener('click', () => f.dossier ? entrer(f) : choisir(f));
        if (!f.dossier) {
            equiperDeTroisPoints(element, f);
            apercuAuSurvol(element, f);
        }
        return element;
    }

    function ligne(f) {
        const d = document.createElement('div');
        d.className = 'exp-ligne';
        d.dataset.apercu = (!f.dossier && importable(f)) ? '1' : '';
        d.innerHTML = `
            <span class="exp-vignette">${icone(f)}</span>
            <span class="exp-infos">
                <span class="exp-nom">${echapper(f.nom)}</span>
                <span class="exp-meta">${meta(f)}</span>
            </span>`;
        d._fichier = f;
        return brancher(d, f);
    }

    // La vue « détails » : un vrai tableau, une colonne par renseignement.
    function enteteDetails() {
        const d = document.createElement('div');
        d.className = 'exp-detail exp-detail-entete';
        // Cinq intitulés pour six colonnes : la dernière, celle des trois
        // points, n'a rien à annoncer.
        d.innerHTML = '<span></span><span>Nom</span><span>Type</span><span>Taille</span><span>Modifié</span>';
        return d;
    }

    function nature(f) {
        if (f.dossier) return 'Dossier';
        const n = (f.nom || '').toLowerCase();
        const ext = n.includes('.') ? n.split('.').pop() : '';
        if (/^image\//.test(f.type || '') || /^(png|jpe?g|gif|webp|svg|bmp|avif)$/.test(ext)) return 'Image';
        if (/pdf/.test(f.type || '') || ext === 'pdf') return 'PDF';
        if (/^audio\//.test(f.type || '') || /^(mp3|wav|ogg|m4a|aac)$/.test(ext)) return 'Son';
        if (/^video\//.test(f.type || '') || /^(mp4|webm|mov|m4v)$/.test(ext)) return 'Vidéo';
        if (/wordprocessing|opendocument\.text|google-apps\.document/.test(f.type || '') || /^(docx|odt)$/.test(ext)) return 'Document';
        if (/spreadsheet|csv/.test(f.type || '') || ext === 'csv') return 'Tableur';
        if (/presentation/.test(f.type || '')) return 'Présentation';
        if (/^text\//.test(f.type || '') || /^(txt|md)$/.test(ext)) return 'Texte';
        return ext ? ext.toUpperCase() : 'Fichier';
    }

    function ligneDetaillee(f) {
        const d = document.createElement('div');
        d.className = 'exp-detail';
        d.dataset.apercu = (!f.dossier && importable(f)) ? '1' : '';
        d.innerHTML = `
            <span class="exp-vignette">${icone(f)}</span>
            <span class="exp-nom">${echapper(f.nom)}</span>
            <span class="exp-colonne">${importable(f) ? nature(f) : 'non pris en charge'}</span>
            <span class="exp-colonne">${f.dossier ? '' : poidsLisible(f.taille)}</span>
            <span class="exp-colonne">${dateLisible(f.date)}</span>`;
        d._fichier = f;
        return brancher(d, f);
    }

    function carte(f) {
        const d = document.createElement('div');
        d.className = 'exp-carte';
        d.dataset.apercu = (!f.dossier && importable(f)) ? '1' : '';
        d.innerHTML = `
            <div class="exp-carte-image"><span class="exp-vignette">${icone(f)}</span></div>
            <div class="exp-nom">${echapper(f.nom)}</div>
            <div class="exp-meta">${meta(f)}</div>`;
        d._fichier = f;
        return brancher(d, f);
    }

    // Les aperçus ne se chargent que pour ce qui est à l'écran : une classe
    // entière de photos ne doit pas partir en téléchargement d'un coup.
    function observerApercus() {
        if (apercus) apercus.disconnect();
        const source = courant();
        if (!source || typeof source.apercu !== 'function' || typeof IntersectionObserver !== 'function') return;

        apercus = new IntersectionObserver((entrees) => {
            entrees.forEach(async (e) => {
                if (!e.isIntersecting) return;
                const noeud = e.target;
                apercus.unobserve(noeud);
                const f = noeud._fichier;
                if (!f || !/^image\//.test(f.type || '') && !/\.(png|jpe?g|gif|webp|bmp|avif)$/i.test(f.nom || '')) return;
                try {
                    const url = await source.apercu(f);
                    if (!url) return;
                    const cible = noeud.querySelector('.exp-vignette');
                    if (cible) cible.innerHTML = `<img src="${url}" alt="">`;
                } catch (err) { /* pas d'aperçu, ce n'est pas grave */ }
            });
        }, { root: el('exp-corps'), rootMargin: '120px' });

        el('exp-corps').querySelectorAll('[data-apercu="1"]').forEach(n => apercus.observe(n));
    }

    // ---------------------------------------------------------
    // LES TROIS POINTS : PRENDRE LE FICHIER SANS PASSER PAR LE TABLEAU
    //
    // « Rajoute trois points à droite du fichier, juste pour le télécharger,
    //   sans passer par Google Drive ni le faire transiter par Au Tableau. »
    //
    // Récupérer un fichier de son nuage demandait jusqu'ici de sortir : ouvrir
    // le site de Drive dans un autre onglet et chercher le même dossier, ou
    // poser le fichier sur le tableau pour le ressortir ensuite. Les trois
    // points font le chemin court — le fichier descend de la source et part
    // droit dans les téléchargements du navigateur. Il ne devient pas une page
    // du tableau et il n'en reste rien ici.
    //
    // PAS DE TROIS POINTS SUR SES PROPRES FICHIERS. Télécharger sur son
    // ordinateur ce qui s'y trouve déjà n'en ferait qu'un doublon : la source
    // « Mon ordinateur » se déclare locale, et s'en passe.
    // ---------------------------------------------------------
    let menuOuvert = null;

    function equiperDeTroisPoints(element, f) {
        const source = courant();
        if (!source || source.local) return;
        const b = document.createElement('button');
        b.className = 'exp-plus';
        b.type = 'button';
        b.textContent = '⋯';
        b.title = 'Enregistrer sur l’ordinateur…';
        b.setAttribute('aria-label', 'Autres actions');
        b.addEventListener('click', (e) => {
            e.stopPropagation();
            cacherLApercu();
            ouvrirLeMenu(b, f);
        });
        element.appendChild(b);
    }

    function fermerLeMenu() {
        if (menuOuvert) { menuOuvert.remove(); menuOuvert = null; }
    }

    function ouvrirLeMenu(bouton, f) {
        const memeFichier = menuOuvert && menuOuvert._fichier === f;
        fermerLeMenu();
        if (memeFichier) return;            // deuxième appui sur les mêmes points : on referme

        const m = document.createElement('div');
        m.className = 'exp-menu';
        m._fichier = f;
        m.innerHTML = `
            <button type="button" class="exp-menu-ligne" data-quoi="enregistrer">⬇️ Enregistrer sur l'ordinateur</button>
            <button type="button" class="exp-menu-ligne" data-quoi="ouvrir">📋 Ouvrir dans le tableau</button>`;
        document.body.appendChild(m);
        menuOuvert = m;

        const r = bouton.getBoundingClientRect();
        const l = m.offsetWidth, h = m.offsetHeight;
        m.style.left = Math.max(8, Math.min(r.right - l, window.innerWidth - l - 8)) + 'px';
        m.style.top = (r.bottom + 4 + h > window.innerHeight ? r.top - h - 4 : r.bottom + 4) + 'px';

        m.querySelector('[data-quoi="enregistrer"]').addEventListener('click', (e) => {
            e.stopPropagation(); fermerLeMenu(); enregistrer(f);
        });
        m.querySelector('[data-quoi="ouvrir"]').addEventListener('click', (e) => {
            e.stopPropagation(); fermerLeMenu(); choisir(f);
        });
    }

    // Les octets traversent bien la page — un fichier protégé par un jeton ne
    // se télécharge pas autrement — mais ils ne s'arrêtent pas : rien n'est
    // importé, rien n'est gardé, et l'adresse temporaire est relâchée ensuite.
    async function enregistrer(fichier) {
        const source = courant();
        if (typeof showToast === 'function') showToast('Téléchargement de « ' + fichier.nom + ' »…');
        try {
            const f = await source.telecharger(fichier);
            const url = URL.createObjectURL(f);
            const a = document.createElement('a');
            a.href = url;
            a.download = f.name || fichier.nom;
            document.body.appendChild(a);
            a.click();
            a.remove();
            setTimeout(() => URL.revokeObjectURL(url), 60000);
            if (typeof showToast === 'function') showToast('« ' + (f.name || fichier.nom) + ' » enregistré');
        } catch (e) {
            if (typeof showToast === 'function') {
                showToast('Téléchargement impossible : ' + (e.message || e));
            }
        }
    }

    // ---------------------------------------------------------
    // L'APERÇU AU SURVOL
    //
    // « En laissant la souris dessus, on peut avoir un aperçu. »
    //
    // Un nom de fichier ne dit pas grand-chose de son contenu — « scan_04.pdf »
    // encore moins que les autres. La souris posée une demi-seconde ouvre une
    // carte : l'image en grand quand la source sait en fournir une (Drive rend
    // même la première page d'un PDF), et de toute façon la nature du fichier,
    // son poids et sa date. Le survol ne coûte rien à celui qui ne s'arrête
    // pas, et rien du tout au doigt : une tablette n'a pas de survol, les trois
    // points y restent accessibles.
    // ---------------------------------------------------------
    const RETARD_APERCU = 420;
    let attenteApercu = null, carteApercu = null;
    let imagesDApercu = new Map();

    function apercuAuSurvol(element, f) {
        element.addEventListener('pointerenter', (e) => {
            if (e.pointerType && e.pointerType !== 'mouse') return;
            clearTimeout(attenteApercu);
            attenteApercu = setTimeout(() => montrerLApercu(element, f), RETARD_APERCU);
        });
        element.addEventListener('pointerleave', () => { cacherLApercu(); });
    }

    function cacherLApercu() {
        clearTimeout(attenteApercu);
        attenteApercu = null;
        if (carteApercu) { carteApercu.remove(); carteApercu = null; }
    }

    // Les images demandées au survol sont gardées le temps du dossier : passer
    // trois fois sur la même ligne ne doit pas la télécharger trois fois.
    function oublierLesApercus() {
        imagesDApercu.forEach(u => { if (u) { try { URL.revokeObjectURL(u); } catch (e) { /* déjà relâchée */ } } });
        imagesDApercu = new Map();
    }

    async function montrerLApercu(element, f) {
        if (!element.isConnected) return;
        cacherLApercu();

        const c = document.createElement('div');
        c.className = 'exp-apercu';
        c.innerHTML = `
            <div class="exp-apercu-image"><span class="exp-vignette">${icone(f)}</span></div>
            <div class="exp-nom">${echapper(f.nom)}</div>
            <div class="exp-meta">${echapper(
                [nature(f), poidsLisible(f.taille), dateLisible(f.date)].filter(Boolean).join(' · '))}</div>`;
        document.body.appendChild(c);
        carteApercu = c;
        placerLApercu(c, element);

        const source = courant();
        if (!source || typeof source.apercu !== 'function') return;
        let url = imagesDApercu.get(f);
        if (url === undefined) {
            try { url = await source.apercu(f, true); } catch (e) { url = null; }
            imagesDApercu.set(f, url || null);
        }
        // On a pu quitter la ligne pendant le chargement : la carte d'alors
        // n'est plus celle d'aujourd'hui, et l'image n'a plus à y entrer.
        if (!url || carteApercu !== c || !c.isConnected) return;
        const image = document.createElement('img');
        image.alt = '';
        image.src = url;
        image.addEventListener('load', () => { if (carteApercu === c) placerLApercu(c, element); });
        const cadre = c.querySelector('.exp-apercu-image');
        cadre.innerHTML = '';
        cadre.appendChild(image);
        cadre.classList.add('exp-apercu-vrai');
    }

    // La carte se pose à droite de la ligne, ou à gauche s'il n'y a plus de
    // place — jamais à cheval sur le bord de l'écran, où elle serait coupée.
    function placerLApercu(c, element) {
        const r = element.getBoundingClientRect();
        const l = c.offsetWidth, h = c.offsetHeight;
        let x = r.right + 12;
        if (x + l > window.innerWidth - 8) x = r.left - l - 12;
        if (x < 8) x = Math.max(8, window.innerWidth - l - 8);
        let y = Math.min(r.top - 8, window.innerHeight - h - 8);
        c.style.left = Math.round(x) + 'px';
        c.style.top = Math.round(Math.max(8, y)) + 'px';
    }

    function entrer(dossier) { chemin.push({ id: dossier.id, nom: dossier.nom }); recherche = ''; charger(); }
    function remonter() { if (chemin.length > 1) { chemin.pop(); recherche = ''; charger(); } }

    // ---------------------------------------------------------
    // LES RACCOURCIS
    // On revient toujours aux trois mêmes dossiers : les cours de l'année, les
    // évaluations, les images. Les retrouver demandait six clics à chaque
    // ouverture. Un raccourci retient la source et le chemin complet ; un clic
    // y ramène. Ils sont propres à ce navigateur, comme le reste des réglages.
    // ---------------------------------------------------------
    function raccourcis() {
        return Array.isArray(etat.raccourcis) ? etat.raccourcis : (etat.raccourcis = []);
    }

    // Un dossier est désigné par sa source et la suite des identifiants
    // traversés : deux dossiers de même nom dans deux sources restent distincts.
    function signature(source, ch) {
        return source + '|' + ch.map(c => c.id).join('|');
    }

    function raccourciCourant() {
        if (chemin.length < 1) return null;
        return raccourcis().find(r => r.signature === signature(etat.source, chemin)) || null;
    }

    function basculerRaccourci() {
        if (!chemin.length) return;
        const sig = signature(etat.source, chemin);
        const liste = raccourcis();
        const i = liste.findIndex(r => r.signature === sig);
        if (i >= 0) {
            const oté = liste.splice(i, 1)[0];
            if (typeof showToast === 'function') showToast('Raccourci retiré : ' + oté.nom);
        } else {
            liste.push({
                signature: sig,
                source: etat.source,
                nom: chemin[chemin.length - 1].nom || 'Dossier',
                chemin: chemin.map(c => ({ id: c.id, nom: c.nom }))
            });
            if (typeof showToast === 'function') {
                showToast('⭐ Raccourci vers « ' + chemin[chemin.length - 1].nom + ' »');
            }
        }
        retenir();
        rendreBarre();
        rendreRaccourcis();
    }

    async function suivreRaccourci(r) {
        // La source d'un raccourci peut ne plus être reliée : on le dit
        // plutôt que d'ouvrir un dossier vide sans explication.
        const src = fournisseurs.find(f => f.cle === r.source);
        if (!src || (!src.dispo() && typeof src.configurer !== 'function')) {
            if (typeof showToast === 'function') {
                showToast('« ' + (src ? src.nom : r.source) + " » n'est pas disponible");
            }
            return;
        }
        if (etat.source !== r.source) { etat.source = r.source; retenir(); rendreSources(); }
        chemin = r.chemin.map(c => ({ id: c.id, nom: c.nom }));
        recherche = '';
        await charger();
    }

    function oublierRaccourci(sig) {
        const liste = raccourcis();
        const i = liste.findIndex(r => r.signature === sig);
        if (i >= 0) liste.splice(i, 1);
        retenir();
        rendreBarre();
        rendreRaccourcis();
    }

    function rendreRaccourcis() {
        const zone = el('exp-raccourcis');
        if (!zone) return;
        const liste = raccourcis();
        if (!liste.length) { zone.style.display = 'none'; zone.innerHTML = ''; return; }
        zone.style.display = 'flex';
        zone.innerHTML = '<span class="exp-raccourcis-titre">RACCOURCIS</span>'
            + liste.map((r, i) => {
                const src = fournisseurs.find(f => f.cle === r.source);
                return `<button class="exp-raccourci" data-i="${i}"
                    title="${echapper(r.chemin.map(c => c.nom).join(' › '))}">
                    <span>${src ? src.icone : '📁'}</span>${echapper(r.nom)}
                    <span class="exp-raccourci-oter" data-sig="${echapper(r.signature)}" title="Retirer ce raccourci">×</span>
                </button>`;
            }).join('');

        zone.querySelectorAll('.exp-raccourci').forEach(b => {
            b.addEventListener('click', (e) => {
                if (e.target.closest('.exp-raccourci-oter')) {
                    oublierRaccourci(e.target.closest('.exp-raccourci-oter').dataset.sig);
                    return;
                }
                suivreRaccourci(liste[parseInt(b.dataset.i, 10)]);
            });
        });
    }

    // Les fichiers repartent par le chemin habituel de l'application : import
    // PDF, image, son, vidéo et document sont déjà écrits, on ne les refait pas.
    function remettre(liste) {
        const entree = document.getElementById('pdf-loader');
        if (!entree) throw new Error("L'import n'est pas disponible");
        const dt = new DataTransfer();
        liste.forEach(f => dt.items.add(f));
        entree.files = dt.files;
        entree.dispatchEvent(new Event('change', { bubbles: true }));
    }

    async function choisir(fichier) {
        message('Ouverture de « ' + echapper(fichier.nom) + ' »…');
        try {
            const f = await courant().telecharger(fichier);
            fermer();
            remettre([f]);
            if (typeof showToast === 'function') showToast('« ' + fichier.nom + ' » importé');
        } catch (e) {
            message('Erreur : ' + echapper(e.message || e), '#d63031');
        }
    }

    // ---------------------------------------------------------
    // OUVERTURE
    // ---------------------------------------------------------
    async function ouvrir(sourceVoulue) {
        construire();
        etat.reduite = false;
        if (typeof window.retirerDuDock === 'function') window.retirerDuDock('explorateur');
        if (sourceVoulue) etat.source = sourceVoulue;
        // Une source indisponible renvoie à l'ordinateur — sauf si elle sait
        // se faire relier : on veut alors son formulaire, pas un repli muet.
        const src = courant();
        if (!src || (!src.dispo() && typeof src.configurer !== 'function')) etat.source = 'ordi';
        el('explorateur').style.display = 'flex';
        appliquerGeometrie();
        rendreSources();
        rendreRaccourcis();
        await ouvrirRacine();
    }

    window.Explorateur = {
        enregistrer(f) {
            if (!f || !f.cle) return;
            const i = fournisseurs.findIndex(x => x.cle === f.cle);
            if (i >= 0) fournisseurs[i] = f; else fournisseurs.push(f);
            if (el('explorateur')) rendreSources();
        },
        ouvrir,
        fermer,
        sources: () => fournisseurs.map(f => ({ cle: f.cle, nom: f.nom, dispo: f.dispo() })),
        importable,
        etat
    };
    window.ouvrirExplorateur = ouvrir;

    Explorateur.enregistrer(ordinateur);

    document.addEventListener('DOMContentLoaded', () => {
        ['btn-drive', 'btn-explorateur'].forEach(id => {
            const b = document.getElementById(id);
            if (b) b.addEventListener('click', () => ouvrir());
        });
    });
})();
